...
def DenseNet(blocks,
    ...
    x = KL.Conv2D(64, 3, activation='relu', padding='same', name='block1/conv1')(x)
    # x = KL.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block1/bn1')(x)
    # x = KL.Activation('relu', name='block1/relu1')(x)x
    x = KL.Conv2D(64, 3, activation='relu', padding='same', name='block1/conv2')(x)
    # x = KL.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block1/bn2')(x)
    # x = KL.Activation('relu', name='block1/relu2')(x)
    x = KL.MaxPooling2D(2, strides=2, name='block1/pool')(x) #output 112*112*64

    # block2
    x = KL.Conv2D(128, 3, activation='relu', padding='same', name='block2/conv1')(x)
    # x = KL.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block2/bn1')(x)
    # x = KL.Activation('relu', name='block2/relu1')(x)
    x = KL.Conv2D(128, 3, activation='relu', padding='same', name='block2/conv2')(x)
    # x = KL.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block2/bn2')(x)
    # x = KL.Activation('relu', name='block2/relu2')(x)
    x = KL.MaxPooling2D(2, strides=2, name='block2/pool')(x) #output 56*56*128

    # block3
    x = KL.Conv2D(256, 3, activation='relu', padding='same', name='block3/conv1')(x)
    # x = KL.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block3/bn1')(x)
    # x = KL.Activation('relu', name='block3/relu1')(x)
    x = KL.Conv2D(256, 3, activation='relu', padding='same', name='block3/conv2')(x)
    # x = KL.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block3/bn2')(x)
    # x = KL.Activation('relu', name='block3/relu2')(x)
    x = KL.Conv2D(256, 3, activation='relu', padding='same', name='block3/conv3')(x)
    # x = KL.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block3/bn3')(x)
    # x = KL.Activation('relu', name='block3/relu3')(x)
    x = KL.MaxPooling2D(2, strides=2, name='block3/pool')(x) #output 28*28*256

    # block4
    x = KL.Conv2D(512, 3, activation='relu', padding='same', name='block4/conv1')(x)
    # x = KL.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block4/bn1')(x)
    # x = KL.Activation('relu', name='block4/relu1')(x)
    x = KL.Conv2D(512, 3, activation='relu', padding='same', name='block4/conv2')(x)
    # x = KL.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block4/bn2')(x)
    # x = KL.Activation('relu', name='block4/relu2')(x)
    x = KL.Conv2D(512, 3, activation='relu', padding='same', name='block4/conv3')(x)
    # x = KL.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block4/bn3')(x)
    # x = KL.Activation('relu', name='block4/relu3')(x)
    x = KL.MaxPooling2D(2, strides=2, name='block4/pool')(x) #output 14*14*512

    # block5
    x = KL.Conv2D(512, 3, activation='relu', padding='same', name='block5/conv1')(x)
    # x = KL.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block5/bn1')(x)
    # x = KL.Activation('relu', name='block5/relu1')(x)
    x = KL.Conv2D(512, 3, activation='relu', padding='same', name='block5/conv2')(x)
    # x = KL.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block5/bn2')(x)
    # x = KL.Activation('relu', name='block5/relu2')(x)
    x = KL.Conv2D(512, 3, activation='relu', padding='same', name='block5/conv3')(x)
    # x = KL.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='block5/bn3')(x)
    # x = KL.Activation('relu', name='block5/relu3')(x)
    x = KL.MaxPooling2D(2, strides=2, name='block5/pool')(x) #output 7*7*512
...
def mask_loss_graph(target_masks,pred_mask):
    #loss=-target_masks*(1-alpha)*((1-pred_mask)*gamma)*tf.log(pred_mask)-\
      #(1-target_masks)*alpha*(pred_mask**gamma)*tf.log(1-pred_mask)
    #pred_mask=pixel_wise_softmax(pred_mask)
    #loss = tf.nn.softmax_cross_entropy_with_logits_v2(logits=pred_mask,labels=target_masks)
    loss=K.binary_crossentropy(target=target_masks,output=pred_mask)
    #loss = cross_entropy(target_masks,pred_mask)
    loss=K.mean(loss)*10
    return loss
##################################################################
